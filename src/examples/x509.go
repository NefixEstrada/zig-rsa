...
func main() {
	...
	for i, test := range objectIdentifierTestData {
		...
		if (err == nil) != test.ok {
			fmt.Printf("#%d: Incorrect error result (did fail? %v, expected: %v)", i, err == nil, test.ok)
		}
		if err == nil {
			if !reflect.DeepEqual(test.out, ret) {
				fmt.Printf("#%d: Bad result: %v (expected %v)", i, ret, test.out)
			}
		}
	}

	if s := ObjectIdentifier([]int{1, 2, 3, 4}).String(); s != "1.2.3.4" {
		fmt.Printf("bad ObjectIdentifier.String(). Got %s, want 1.2.3.4", s)
	}
}
...
func parseField(v reflect.Value, bytes []byte, initOffset int, params fieldParameters) (offset int, err error) {
	...
	if !matchAnyClassAndTag && (t.class != expectedClass || t.tag != expectedTag) ||
		...
		fmt.Println("=++++++======")
		fmt.Println(t)
		fmt.Println(getUniversalType(fieldType))
		fmt.Println(matchAnyClassAndTag)
		fmt.Println(expectedClass)
	...
	case *ObjectIdentifier:
		fmt.Println("parsing object identifier!")
	...
	case reflect.Struct:
		...
		for i := 0; i < structType.NumField(); i++ {
			...
			fmt.Println("-------")
			fmt.Println(innerBytes)
			fmt.Println(innerOffset)
