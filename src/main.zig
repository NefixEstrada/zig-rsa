...
fn powMod(x: std.math.big.int.Managed, y: std.math.big.int.Managed, m: std.math.big.int.Const) anyerror!std.math.big.int.Managed {
    if (m.eqZero()) {
        return error.DivideByZero;
    }

    var one = try std.math.big.int.Managed.init(x.allocator);
    defer one.deinit();
    try one.set(1);

    if (y.eq(one)) {
        var temp = try std.math.big.int.Managed.init(x.allocator);
        defer temp.deinit();

        var result = try std.math.big.int.Managed.init(x.allocator);
    ...
    var two = try std.math.big.int.Managed.init(x.allocator);
    defer two.deinit();
    try two.set(2);

    var y_half = try std.math.big.int.Managed.init(x.allocator);
    defer y_half.deinit();
    ...
    if (temp.eq(one)) {
        ...
        var temp2 = try std.math.big.int.Managed.init(x.allocator);
        defer temp2.deinit();
...
test "powMod should work" {
    var x = try std.math.big.int.Managed.init(std.testing.allocator);
    defer x.deinit();
    try x.set(2);

    var y = try std.math.big.int.Managed.init(std.testing.allocator);
    defer y.deinit();
    try y.set(99999999);

    var z = try std.math.big.int.Managed.init(std.testing.allocator);
    defer z.deinit();
    try z.set(147);
...
fn encrypt(allocator: std.mem.Allocator, pub_key: PublicKey, m: std.math.big.int.Managed) !std.math.big.int.Managed {
    var e = try std.math.big.int.Managed.init(allocator);
    try e.set(@intCast(i64, pub_key.e));
    defer e.deinit();

    return try powMod(m, e, pub_key.n.toConst());
}

fn decrypt(_: std.rand.Random, priv_key: PrivateKey, c: std.math.big.int.Managed) !std.math.big.int.Managed {
    ...
    return try powMod(c, priv_key.d, priv_key.n.toConst());
}
...
pub fn encryptOaep(allocator: std.mem.Allocator, h: anytype, random: std.rand.Random, pub_key: PublicKey, msg: []const u8, label: []const u8) ![]const u8 {
    ...
    try setBytes(allocator, &m, em);

    var c = try encrypt(allocator, pub_key, m);
    defer c.deinit();

    return try toBytes(allocator, c);
}
...
test "encryptOaep should work as expected" {
    ...
    const testEncryptOaep = struct {
        fn testEncryptOaep(modulus: []const u8, e: i32, seed: []const u8, in: []const u8, expected: []const u8) !void {
            var n = try std.math.big.int.Managed.init(std.testing.allocator);
            defer n.deinit();
...
fn constantTimeCompare(x: []const u8, y: []const u8) usize {
...
fn constantTimeSelect(v: usize, x: usize, y: usize) usize {
    if (v == 0) return y;
    if (v == 1) return x;
...
fn constantTimeByteEq(x: u8, y: u8) usize {
    if (x == y) return 1;
    return 0;
}
...
pub fn decryptOaep(allocator: std.mem.Allocator, h: anytype, random: std.rand.Random, priv_key: PrivateKey, ciphertext: []const u8, label: []const u8) ![]const u8 {
    ...
    var c = try std.math.big.int.Managed.init(allocator);
    defer c.deinit();
    try setBytes(allocator, &c, ciphertext_buf);
    ...
    var buf = try toBytes(allocator, m);
    defer allocator.free(buf);
    for (buf) |b, i| {
        em[i] = b;
    }

    var last_byte_is_zero = constantTimeByteEq(em[em.len - 1], 0);
    ...
    var l_hash2_good = constantTimeCompare(l_hash[0..H.digest_length], l_hash2);
    ...
    for (rest) |_, i| {
        var equals0 = constantTimeByteEq(rest[i], 0);
        var equals1 = constantTimeByteEq(rest[i], 1);
        index = constantTimeSelect(looking_for_index & equals1, i, index);
        looking_for_index = constantTimeSelect(equals1, 0, looking_for_index);
        invalid = constantTimeSelect(looking_for_index & ~equals0, 1, invalid);
    }
...
test "decryptOaep should work as expected" {
    const testDecryptOaep = struct {
        fn testDecryptOaep(modulus: []const u8, e: i32, d_hex: []const u8, ciphertext: []const u8, expected: []const u8) !void {
            ...
            var d = try std.math.big.int.Managed.init(std.testing.allocator);
            defer d.deinit();
